...
| 인증 구분 | 환경변수 | 사용 대상 |
| --- | --- | --- |
| 중소벤처24 OpenAPI | `SMES24_OPENAPI_TOKEN` | 중소벤처24 공고정보, 증명서 확인 |
...
| 공고 연결 개선 | 중소벤처24 최신 공고 API를 사용해 실제 접수 기간과 상세 URL 연결 |
| 추천점수 정교화 | 업종, 업력, 자산, 종업원 수, 지역, 자금종류별 수혜 패턴을 추천점수에 반영 |
| 승인가능성 피처 확장 | 과거 지원 비중, 전환율, 규모 적합도, 재무 지표를 모델 feature 후보로 추가 |
| 리스크 판정 강화 | 융자제외 업종, 필요 인증 미충족, 과거 수혜 패턴 이탈을 리스크로 표시 |
...
| P1 | 공고 연결 | 중소벤처24 공고정보 연계 API | 추천 자금과 실제 공고 상세/신청 URL 연결 |
| P1 | 기본 수혜 패턴 | 업종별 지원 현황, 업력별 지원현황, 자금종류별 융자 현황 | 현재 추천점수와 승인가능성 모델의 핵심 보강 데이터 |
...
| P3 | 인증 검증 | 중소벤처24 증명서 일괄 확인 서비스 API | 공고별 필요 인증 충족 여부 확인 |

## 4. API 인벤토리

| API | 인증 | 가이드 | 주요 활용 | 저장 후보 |
| --- | --- | --- | --- | --- |
| 중소벤처24_공고정보 연계 API | `SMES24_OPENAPI_TOKEN` | `docs/api_guides/smes24-notice-linkage-api-guide-ai-context.md` | 최신 공고 수집, 접수기간/상세 URL/신청 URL 연결 | `external_notices` |
| 중소벤처24_증명서 일괄 확인 서비스 API | `SMES24_OPENAPI_TOKEN` | `docs/api_guides/smes24-certificate-batch-verification-api-guide-ai-context.md` | 벤처/이노비즈/메인비즈 등 인증 보유 여부 확인 | `company_certificates` |
...
중소벤처24 API는 정책 공고와 필요 인증 검증에 사용합니다.
...
| 증명서 일괄 확인 API | `GET/POST https://www.smes.go.kr/api/certificates/{certificateCode}` | `Authorization: Bearer {SMES24_OPENAPI_TOKEN}`, `certificateCode`, `bizno` | `bizno`, `crtfCd`, `crtfNm`, `resultCd`, `resultMsg`, 인증서 유효 정보 | 사업자등록번호 10자리 사용. 공고 필요 인증과 비교 |

공고 신청 화면 개선 흐름:

1. 공고 API를 배치 수집해 `external_notices`에 저장합니다.
2. 추천 자금명, 업종, 지역, 업력, 매출, 종업원 수, 인증 조건으로 공고 후보를 필터링합니다.
3. TOP3 비교 또는 공고 신청 단계에서 실제 `pblancDtlUrl`과 `reqstLinkInfo`를 표시합니다.
...
| 기업 재무정보 | `getSummFinaStat_V2`, `getBs_V2`, `getIncoStat_V2` | `serviceKey`, `crno`, `bizYear`, `resultType=json` | `enpSaleAmt`, `enpBzopPft`, `enpCrtmNpf`, `enpTastAmt`, `enpTdbtAmt`, `enpTcptAmt`, `fnclDebtRto` | 매출/자산/부채/부채비율 입력 검증, 승인가능성 feature |
...
| 기업입력 | 중소벤처24 증명서 확인 | 벤처, 이노비즈, 메인비즈 등 인증 보유 여부 검증 |
| 분석 | 융자제외 대상 업종 | 추천 전 hard eligibility rule 적용 |
| 분석 | 업종별, 업력별, 자금종류별 지원 현황 | 승인가능성 모델 feature와 과거 수혜 패턴 점수 |
...
| 공고신청 | 중소벤처24 공고정보 | 실제 공고 상세 URL, 신청 URL, 접수기간, 필요 인증 표시 |
...
| `company_certificates` | 사업자등록번호별 인증서 조회 결과 |
...
- 중소벤처24 공고 API의 필요 인증 정보와 증명서 확인 API 결과를 연결합니다.
...
6. 중소벤처24 공고정보 수집 후 공고 신청 단계의 검색 링크를 실제 공고 URL 기반으로 개선
7. 특화자금 API를 정책우선도/세부 자금 추천 설명에 순차 반영
8. 인증서 확인 API로 필요 인증 미충족 경고 추가
...
| 인증서 코드 | 중소벤처24 증명서 API의 실제 코드와 응답 필드 |
